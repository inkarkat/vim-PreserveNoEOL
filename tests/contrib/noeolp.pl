	...
	eval
	{
	    ...
	    my $file = "windows.txt";
	    if (! -w $file == 1) {
		printf("echomsg 'need to undo read-only'\n");
		...
		if ($perms2 ne $perms) {
		    printf("echomsg 'I need the read-only fix'\n");
		    printf("perl chmod $perms, '$file' or die \"Can't restore read-only flag: \$!\"\n");
		} else {
		    printf("echomsg 'No need for the read-only fix'\n");
		}
	    }
	};
	$@ =~ s/'/''/g;
